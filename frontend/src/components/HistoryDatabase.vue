<template>
  ...
  >
    <!-- Background decoration: Tech grid lines (only shown when there are projects) -->
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <!-- Card Title (uses first 20 chars of simulation requirement as title) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" require live execution and do not support history playback</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Records pending target state

// Card layout config - adjusted for wider aspect ratio
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record latest state regardless of animation)
        ...
        // If animating, just record state, wait for animation to finish
        ...
        // Use debounce for state transition, prevent rapid flicker
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // After animation completes, release lock and check for pending state changes
          setTimeout(() => {
            isAnimating = false

            // After animation ends, check if there's a new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Delay briefly before executing, avoid too-fast switching
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
...
// Listen for route changes, reload data when returning to home
...
/* ===== Historical replay detail modal styles ===== */
.modal-overlay {
  position: fixed;
  top: 0;
  left: 0;
