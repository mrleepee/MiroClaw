  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" title="Refresh graph" aria-label="Refresh graph">
          <span class="icon-refresh" :class="{ 'spinning': loading }"><MiroClawIcons icon="refresh-cw" :size="14" /></span>
          <span class="btn-text">Refresh</span>
        </button>
        <button class="tool-btn" @click="$emit('toggle-maximize')" title="Maximize/Restore" aria-label="Toggle maximize">
          <MiroClawIcons icon="maximize-2" :size="14" />
        </button>
      </div>
    </div>
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        <svg ref="graphSvg" class="graph-svg" role="img" aria-label="Knowledge graph visualization"></svg>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content still processing, recommend manual refresh</span>
      ...
      <div v-else class="graph-state">
        <div class="empty-icon"><MiroClawIcons icon="hexagon" :size="48" /></div>
...
// Watch isSimulating change to detect simulation end
...
const renderGraph = () => {
  ...
  // Process edge data, compute edge count and index for each node pair
  const edgePairCount = {}
  ...
  // Track which edge is currently being processed for each node pair
  const edgePairIndex = {}
  ...
  tempEdges.forEach(e => {
    ...
    // Compute curvature: spread apart for multiple edges, straight line for single edge
    let curvature = 0
    if (totalCount > 1) {
      // Uniform curvature distribution for clear distinction
      // Curvature range increases with edge count, larger range for more edges
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // If edge direction is reversed from normalized direction, flip curvature
      // This ensures all edges are distributed in same reference frame, avoiding overlap from direction differences
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add attraction to center to cluster isolated node groups to center area
  ...
  const getLinkPath = (d) => {
    ...
    // Compute curve control point - dynamically adjust based on edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Offset perpendicular to connection line, calculated by distance ratio for visible curve
  ...
  // Store references for external visibility control
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if truly dragging (movement exceeds threshold)
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)

        if (!d._isDragging && distance > 3) {
          // Only restart simulation when true drag is detected
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only let simulation gradually stop if truly dragged
        if (d._isDragging) {
...
.graph-panel {
  ...
  width: 100%;
  height: 100%;
  background-color: var(--color-bg-surface, #FAFAFA);
  background-image: radial-gradient(var(--color-border, #D0D0D0) 1.5px, transparent 1.5px);
...
.panel-header {
  ...
  align-items: center;
  background: linear-gradient(to bottom, var(--color-bg-primary-transparent, rgba(255,255,255,0.95)), transparent);
  pointer-events: none;
}
...
.graph-legend {
  ...
  bottom: 24px;
  left: 24px;
  background: var(--color-bg-primary-transparent, rgba(255,255,255,0.95));
  padding: 12px 16px;
  border-radius: var(--radius-md, 6px);
...
/* Hint styles after simulation ends */
...
/* Dark mode adjustments for D3 SVG elements */
