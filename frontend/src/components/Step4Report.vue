    ...
    <div class="main-split-layout">
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <!-- Next Step Button - Shows when complete -->
            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated, but section may not be complete) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Show special indicator when it's the final answer -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // Wait for DOM update, then adjust scroll to keep button in same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract statistics - matches "Related prediction facts: X" format
    ...
    // Extract sub-questions - complete extraction, no limit
    ...
    // Extract core entities - complete extraction, includes summary and related facts count
    ...
    // Extract relationship chains - complete extraction, no limit
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract currently valid facts - complete extraction, no limit
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Example: 1. **Alumni_345 (index=1)**: As a Wuhan University alumni...
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Total: ${props.result.entities.length} entities`)
          ]),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Total: ${props.result.subQueries.length} questions`)
          ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Maintain independent platform selection state per question-answer pair
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "问题X：" format (backend question numbering)
      // 2. "1. " or "\n1. " (number+dot, old format compatible)
      let matches = []
      let match

      // Try question numbering format first
      const cnPattern = /(?:^|[\r\n]+)问题(\d+)[：:]\s*/g
    ...
    // Check if question has dual-platform answers (filter placeholders)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers (non-placeholder) with different content
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Multi-platform switch button (only shown when there are real dual-platform answers)
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `Total: ${props.result.nodes.length} nodes`)
          ]),
...
const renderMarkdown = (content) => {
  ...
  // Remove leading secondary titles (## xxx) since section title is already displayed in outer layer
  ...
  // Clean <p><br> followed by block elements (excess blank lines)
  ...
  // Fix non-consecutive ordered list numbering: maintain incrementing when single <ol> items are separated by paragraph content
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after "最终答案:" (backend final answer marker)
  ...
  // If content is long and contains markdown format, try to remove thinking process
