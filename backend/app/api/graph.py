...
def build_graph():
    ...
    try:
        ...
        # Validate configuration (Neo4j required for LocalGraphService)
        config_errors = Config.validate()
        if config_errors:
            logger.error(f"Configuration errors: {config_errors}")
            return jsonify({
                "success": False,
                "error": "Configuration error: " + "; ".join(config_errors)
            }), 500
        ...
        def build_task():
            ...
            try:
                ...
                # LocalGraphService processes immediately via LLM, no need to wait
                builder._add_text_batches(
                    graph_id,
                    chunks,
                    batch_size=3,
