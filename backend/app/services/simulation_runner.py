...
Runs simulations in the background and records each agent's actions, with support for real-time status monitoring
"""
...
class SimulationRunState:
    ...
    # Platform completion status (detected from the simulation_end event in actions.jsonl)
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        enable_graph_memory_update: bool = False,  # Whether to update activity to the Zep graph
        ...
        Args:
            ...
            platform: Execution platform (twitter/reddit/parallel)
            max_rounds: Maximum simulation rounds (optional, used to truncate overly long simulations)
            enable_graph_memory_update: Whether to dynamically update agent activity to the Zep graph
        ...
        if not os.path.exists(config_path):
            raise ValueError("Simulation config does not exist, please call the /prepare endpoint first")
        ...
        # Truncate if a maximum number of rounds was specified
        ...
        # Create the updater if graph memory updates are enabled
        if enable_graph_memory_update:
            if not graph_id:
                raise ValueError("graph_id must be provided when graph memory updates are enabled")
            
            try:
                SimulationGraphManager.create_updater(simulation_id, graph_id)
        ...
        try:
            ...
            cmd = [
                ...
                script_path,
                "--config", config_path,  # Use absolute config file path
            ]
            
            # Add max rounds to CLI args if specified
            if max_rounds is not None and max_rounds > 0:
                cmd.extend(["--max-rounds", str(max_rounds)])
            
            # Create the main log file to avoid blocking when stdout/stderr pipe buffers fill up
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')
            
            # Set subprocess environment variables to ensure UTF-8 encoding on Windows
            # This fixes cases where third-party libraries (such as OASIS) read files without specifying encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+ support: make all open() calls use UTF-8 by default
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensure stdout/stderr use UTF-8
            
            # Set the working directory to the simulation directory (databases and similar files will be created here)
            # Use start_new_session=True to create a new process group so all child processes can be terminated via os.killpg
            ...
            # Save file handles so they can be closed later
            cls._stdout_files[simulation_id] = main_log_file
            cls._stderr_files[simulation_id] = None  # No separate stderr handle is needed anymore
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # Read logs one final time after the process exits
            if os.path.exists(twitter_actions_log):
            ...
            if exit_code == 0:
                ...
                state.runner_status = RunnerStatus.FAILED
                # Read error details from the main log file
                main_log_path = os.path.join(sim_dir, "simulation.log")
                error_info = ""
                try:
            ...
            if cls._graph_memory_enabled.get(simulation_id, False):
                try:
                    SimulationGraphManager.stop_updater(simulation_id)
    ...
    def _read_action_log(
        ...
        if graph_memory_enabled:
            graph_updater = SimulationGraphManager.get_updater(state.simulation_id)
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If both platforms were run, both must complete
                                    all_completed = cls._check_all_platforms_completed(state)
                                    if all_completed:
                                        state.runner_status = RunnerStatus.COMPLETED
                                        state.completed_at = datetime.now().isoformat()
                                        logger.info(f"All enabled platforms completed: {state.simulation_id}")
                                
                                # Update round information (from the round_end event)
                                elif event_type == "round_end":
                                    round_num = action_data.get("round", 0)
                                    simulated_hours = action_data.get("simulated_hours", 0)
                                    
                                    # Update per-platform round counts and simulated time
                                    if platform == "twitter":
                                    ...
                                    # The overall round count is the maximum across both platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # The overall simulated time is the maximum across both platforms
                            ...
                            # Send activity to Zep if graph memory updates are enabled
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        # Check which platforms are enabled by whether the files exist
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Terminate a process and its child processes cross-platform.
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                try:
                    ...
                    # Force terminate
                    logger.warning(f"Process did not respond, force terminating: {simulation_id}")
                    subprocess.run(
            ...
            # Because start_new_session=True is used, the process group ID equals the main process PID
            ...
            try:
                ...
                # If it is still running after the timeout, force termination with SIGKILL
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Determine platform: prefer the platform in the record, otherwise use the default platform
    ...
    def get_all_actions(
        ...
        """
        Get the full action history across all platforms (without pagination limits).
        ...
        # Read the Twitter action file (automatically set platform to twitter based on the file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read the Reddit action file (automatically set platform to reddit based on the file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
        ...
        if not actions:
            ...
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Old-format files should already contain a platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation runtime logs (used to force a fresh restart).
        ...
        # Clean action logs inside platform directories
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check whether there is anything to clean up to avoid unnecessary log output
        ...
        try:
            SimulationGraphManager.stop_all()
        except Exception as e:
            logger.error(f"Failed to stop graph memory updaters: {e}")
        cls._graph_memory_enabled.clear()
        
        # Copy the dictionary to avoid modifying it during iteration
            ...
            try:
                if process.poll() is None:  # Process is still running
                    ...
                    try:
                        ...
                        # The process may already be gone, try terminating it directly
                        try:
                            process.terminate()
                    ...
                    if state:
                        ...
                        state.reddit_running = False
                        state.completed_at = datetime.now().isoformat()
                        state.error = "Simulation was terminated because the server shut down"
                        cls._save_run_state(state)
                    
                    # Also update state.json and set its status to stopped
                    try:
                        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def register_cleanup(cls):
        ...
        # Outside debug mode, this environment variable is absent and cleanup still needs to be registered
        ...
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # Mark as registered to prevent the child process from trying again
            return
        
        # Save the original signal handlers
        original_sigint = signal.getsignal(signal.SIGINT)
        original_sigterm = signal.getsignal(signal.SIGTERM)
        # SIGHUP exists only on Unix systems (macOS/Linux), not on Windows
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                # If the original handler is not callable (such as SIG_DFL), use the default behavior
        ...
        try:
            ...
            # Not in the main thread, so only atexit can be used
            logger.warning("Unable to register signal handlers because this is not the main thread; using atexit only")
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Status detail dictionary containing status, twitter_available, reddit_available, and timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None: In dual-platform simulations, interview both platforms and return a merged result
        ...
        Raises:
            ...
            TimeoutError: Waiting for the response timed out
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation does not exist: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            simulation_id: Simulation ID
            interviews: Interview list, where each item contains {"agent_id": int, "prompt": str, "platform": str(optional)}
            platform: Default platform (optional, overridden by each interview item's platform)
                - "twitter": By default, interview only the Twitter platform
                - "reddit": By default, interview only the Reddit platform
                - None: In dual-platform simulations, interview both platforms for each agent
            timeout: Timeout in seconds
    ...
    def interview_all_agents(
        ...
        Use the same question to interview every agent in the simulation.
    ...
    def close_simulation_env(
        ...
        Send a close-environment command to the simulation so it can gracefully exit command-wait mode.
        ...
        try:
            ...
            # A timeout may mean the environment is already shutting down
            return {
                "success": True,
                "message": "Close-environment command sent (waiting for a response timed out, the environment may be shutting down)"
            }
    ...
    def get_interview_history(
        ...
        Args:
            ...
            platform: Platform type (reddit/twitter/None)
                - "reddit": Get history only from the Reddit platform
                - "twitter": Get history only from the Twitter platform
                - None: Get all history from both platforms
            agent_id: Specified agent ID (optional, only get history for that agent)
            limit: Maximum number of records returned per platform
        ...
        # If multiple platforms were queried, limit the total number of records
