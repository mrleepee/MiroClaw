"""
Graph search tool service
Encapsulates graph search, node retrieval, edge queries, and related tools for the Report Agent

Core retrieval tools (optimized):
1. InsightForge (Deep Insight Retrieval) - the most powerful hybrid retrieval, automatically generates sub-questions and searches across multiple dimensions
...
class InsightForgeResult:
    ...
    Includes retrieval results for multiple sub-questions and combined analysis
    """
    query: str
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter junk content that contains question numbering (Question 1-9)
                skip = False
                for d in '123456789':
                    if (
                        f'Question {d}' in clean_quote
                        or f'Question{d}' in clean_quote
                        or f'\u95ee\u9898{d}' in clean_quote
                    ):
                        skip = True
                        break
                if skip:
                    continue
                # Truncate overly long content at a sentence boundary instead of hard cutting
                if len(clean_quote) > 150:
                    dot_positions = [pos for pos in (clean_quote.find('.', 80), clean_quote.find('\u3002', 80)) if pos > 0]
                    dot_pos = min(dot_positions) if dot_positions else -1
                    if dot_pos > 0:
...
class InterviewResult:
    ...
    # Reasoning for selecting the Agents
...
class GraphSearchService:
    """
    Graph search and retrieval tool service

    [Core retrieval tools - optimized]
    1. insight_forge - Deep insight retrieval (the most powerful, automatically generates sub-questions and searches across multiple dimensions)
    ...
    4. interview_agents - In-depth interviews (interviews simulated Agents to gather multi-perspective viewpoints)
    ...
    def __init__(self, graph_id: str, llm_client: Optional[LLMClient] = None):
        self.graph_id = graph_id
        self.graph_service = get_graph_service()
        # LLM client used by InsightForge to generate sub-questions
        self._llm_client = llm_client
        logger.info(f"GraphSearchService initialized: graph_id={graph_id}")
    ...
    def search_graph(
        ...
        Use LocalGraphService search (vector similarity + keyword matching)
        to search for relevant information in the graph.
        ...
        try:
            ...
            logger.warning(f"Search API failed, falling back to local search: {str(e)}")
    ...
    def _local_search(
        ...
        Fetch all edges/nodes, then perform local keyword matching
        ...
        try:
            if scope in ["edges", "both"]:
                # Fetch all edges and match them
            ...
            if scope in ["nodes", "both"]:
                # Fetch all nodes and match them
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        ...
        # Use LocalGraphService node API
    ...
    def get_all_edges(self, graph_id: str, include_temporal: bool = True) -> List[EdgeInfo]:
        ...
        # Use LocalGraphService edge API
    ...
    def get_node_detail(self, node_uuid: str) -> Optional[NodeInfo]:
        ...
        try:
            node = self.graph_service.node.get(uuid_=node_uuid)
    ...
    def get_simulation_context(
        ...
        Search comprehensively for all information related to the simulation requirement
        
        Args:
            graph_id: Graph ID
            simulation_requirement: Simulation requirement description
            limit: Quantity limit for each category of information
        ...
        # Filter entities with an actual type (not pure Entity nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        The most powerful hybrid retrieval function, automatically decomposing the question and searching across multiple dimensions:
        1. Use the LLM to break the question into multiple sub-questions
        2. Perform semantic search for each sub-question
        3. Extract related entities and fetch their detailed information
        ...
        entity_insights = []
        node_map = {}  # Used later when building relationship chains
        ...
        for uuid in list(entity_uuids):  # Process all entities without truncation
            ...
            try:
                ...
                if node:
                    ...
                    # Fetch all facts related to this entity (without truncation)
                    related_facts = [
    ...
    def _generate_sub_queries(
        ...
        """
        system_prompt = """You are a professional question analysis expert. Your task is to break a complex question into multiple sub-questions that can be independently observed in the simulated world.
...
3. The sub-questions should be relevant to the simulation scenario
...
Return the list of sub-questions in JSON format."""
    ...
    def panorama_search(
        ...
        2. Fetch all edges (including expired/invalid ones)
        3. Categorize current valid information and historical information
        
        This tool is suitable for scenarios where you need to understand the full picture of an event and trace how it evolved.
        ...
        # Fetch all edges (including temporal information)
        ...
        # Rank by relevance based on the query
        query_lower = query.lower()
        keywords = [w.strip() for w in query_lower.replace(',', ' ').replace('，', ' ').split() if len(w.strip()) > 1]
    ...
    def interview_agents(
        ...
        2. Use the LLM to analyze the interview requirement and intelligently select the most relevant Agents
        3. Use the LLM to generate interview questions
        4. Call the /api/simulation/interview/batch endpoint to perform real interviews (both platforms at once)
        5. Consolidate all interview results and generate an interview report
        
        [Important] This feature requires the simulation environment to be running (the OASIS environment must not be shut down)
        
        [Use cases]
        - When you need to understand opinions on an event from different roles
        - When you need to collect opinions and viewpoints from multiple parties
        - When you need real responses from simulated Agents (not LLM-simulated answers)
        
        Args:
            simulation_id: Simulation ID (used to locate persona files and call the interview API)
            interview_requirement: Interview requirement description (unstructured, for example "understand students' views on the event")
        ...
        # Step 2: Use the LLM to select Agents to interview (returns an agent_id list)
        ...
        # Add an optimized prefix to constrain the Agent response format
        INTERVIEW_PROMPT_PREFIX = (
            "You are being interviewed. Please answer the following questions directly in plain text based on your persona, all of your past memories, and your actions.\n"
            ...
            "4. Answer each question in order by number, and start each answer with \"Question X:\" (X is the question number)\n"
            "5. Separate the answers to each question with a blank line\n"
            "6. The answers must contain substantive content; answer each question with at least 2-3 sentences\n\n"
        )
        optimized_prompt = f"{INTERVIEW_PROMPT_PREFIX}{combined_prompt}"
        
        # Step 4: Call the real interview API (platform not specified, both platforms are interviewed by default)
        try:
            # Build the batch interview list (platform not specified, both platforms are interviewed)
            interviews_request = []
            ...
            logger.info(f"Calling the batch interview API (both platforms): {len(interviews_request)} Agents")
            
            # Call SimulationRunner's batch interview method (no platform passed, both platforms interviewed)
            api_result = SimulationRunner.interview_agents_batch(
                simulation_id=simulation_id,
                interviews=interviews_request,
                platform=None,  # Platform not specified, both platforms interviewed
            ...
            # Both-platform mode response format: {"twitter_0": {...}, "reddit_0": {...}, "twitter_1": {...}, ...}
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Clean any possible tool-call JSON wrappers
                ...
                clean_text = re.sub('(?:Question\\s*\\d+|\u95ee\u9898\\d+)[：:]?\\s*', '', clean_text, flags=re.IGNORECASE)
                ...
                # Strategy 2 (supplemental): long text inside properly paired quotation marks
                if not key_quotes:
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may be shut down. Please make sure OASIS is running."
            return result
        except Exception as e:
        ...
        if os.path.exists(twitter_profile_path):
            try:
                with open(twitter_profile_path, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        # Convert the CSV format to the unified format
                        profiles.append({
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                ...
                - selected_indices: Index list for the selected Agents (used for API calls)
        ...
        system_prompt = """You are a professional interview planning expert. Your task is to choose the most suitable interview subjects from a list of simulated Agents based on the interview requirement.

Selection criteria:
1. The Agent's identity/profession is relevant to the interview topic
2. The Agent may hold a unique or valuable viewpoint
3. Select a diverse set of perspectives (such as supporters, opponents, neutrals, professionals, etc.)
...
Selectable Agent list ({len(agent_summaries)} total):
{json.dumps(agent_summaries, ensure_ascii=False, indent=2)}

Please select up to {max_agents} Agents who are the most suitable for interview, and explain the selection rationale."""
        ...
        try:
            ...
            # Get the full information for the selected Agents
            selected_agents = []
            valid_indices = []
    ...
    def _generate_interview_questions(
        ...
        system_prompt = """You are a professional reporter/interviewer. Generate 3-5 in-depth interview questions based on the interview requirement.

Question requirements:
1. Open-ended questions that encourage detailed responses
2. Different roles should be able to give different answers
3. Cover multiple dimensions such as facts, opinions, and feelings
4. Use natural language, like a real interview
5. Keep each question under 50 words and concise
...
- Use plain-text paragraphs, with blank lines between sections
