"""
OASIS dual-platform parallel simulation bootstrap script
...
- After simulation completion, do not close the environment immediately, enter command waiting mode
...
# This fixes the issue where OASIS third-party libraries do not specify encoding when reading files
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout to UTF-8 (fixes console Chinese character garbling)
    ...
    # Note: This needs to be set at Python startup time, setting at runtime may not work
    # So we need to monkey-patch the built-in open function
    import builtins
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        Wrapper for open() function that defaults to UTF-8 encoding for text mode
        This fixes the issue where third-party libraries (like OASIS) do not specify encoding when reading files
        """
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally don't set max_tokens and let the model decide)"""
...
# Memory safety for OASIS agents.
#
# Problem: OASIS's SocialAgent inherits CAMEL's ChatAgent with window_size=None
# (unlimited message history). Worse, CAMEL defaults token_limit to 999,999,999
# for unrecognised models like MiniMax-M2.7, so token-based pruning also never
# fires. The result: each agent's in-memory message list grows without bound,
# causing context window overflow errors at the API and eventually OOM (40GB+
# observed at round 44/72 with 182 agents).
#
# Fix: After agent graph creation, replace each agent's memory with a
# ChatHistoryMemory that has both:
#   1. A realistic token_limit matching the actual model context window, so
#      CAMEL prunes messages that wouldn't fit in the API call anyway.
#   2. A message_window_size safety backstop, preventing pathological cases
#      where token counting is inaccurate from accumulating unbounded objects.
#
# With a 205K context window and ~2K tokens per round, the token limit alone
# allows ~100 rounds of history — more than a full 72-round simulation. The
# message window (150 messages ≈ 75 rounds) is a safety backstop that should
# rarely activate.

# MiniMax-M2.7 context window: 204,800 tokens.
# Reserve 4,800 for the system prompt + response headroom.
AGENT_CONTEXT_TOKEN_LIMIT = 200_000

# Safety backstop: max messages in memory (150 messages ≈ 75 rounds).
# Should rarely activate — the token limit is the primary constraint.
AGENT_MEMORY_WINDOW_SIZE = 150


def apply_memory_limits(agent_graph, platform_name: str) -> None:
    """Replace each agent's ChatHistoryMemory with a token-limited, windowed version.

    OASIS creates agents with window_size=None and token_limit=999M (for
    unrecognised models). This causes unbounded memory growth. We replace the
    memory with proper limits so CAMEL prunes old messages before they overflow
    the context window or exhaust RAM.
    """
    from camel.memories.context_creators import ScoreBasedContextCreator

    count = 0
    for _agent_id, agent in agent_graph.get_agents():
        old_creator = agent.memory.get_context_creator()
        # Build a new context creator with the real token limit.
        # Preserve the existing token counter from the model backend.
        new_creator = ScoreBasedContextCreator(
            token_counter=old_creator.token_counter,
            token_limit=AGENT_CONTEXT_TOKEN_LIMIT,
        )
        windowed_memory = ChatHistoryMemory(
            context_creator=new_creator,
            window_size=AGENT_MEMORY_WINDOW_SIZE,
        )
        agent.memory = windowed_memory
        count += 1
    print(f"[{platform_name}] Applied memory limits to {count} agents "
          f"(token_limit={AGENT_CONTEXT_TOKEN_LIMIT:,}, window={AGENT_MEMORY_WINDOW_SIZE} msgs)")
...
class ParallelIPCHandler:
    ...
    Manages environments for two platforms, handles Interview commands
    """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None/unspecified: Interview on both platforms, return integrated result
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        """
        Handle batch interview commands
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows displaying actual entity names in actions.jsonl instead of codes like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from database and supplement with complete context information
    ...
    Returns:
        ...
        - actions_list: List of actions, each element contains agent_id, agent_name, action_type, action_args (with context information)
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Supplement context information (post content, user names, etc.)
...
def _enrich_action_context(
    ...
    """
    Supplement context information for actions (post content, user names, etc.)
    ...
    try:
        ...
        # Quote post: supplement original post content, author, and quote comment
            ...
            # Get quote comment content (quote_content)
            if new_post_id:
                cursor.execute("""
        ...
        # Create comment: supplement information of the commented post
        ...
        # Failure to supplement context does not affect main flow
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Support dual LLM configuration for speedup during parallel simulations:
    - General configuration: LLM_API_KEY, LLM_BASE_URL, LLM_MODEL_NAME
    - Accelerated configuration (optional): LLM_BOOST_API_KEY, LLM_BOOST_BASE_URL, LLM_BOOST_MODEL_NAME

    If accelerated LLM is configured, different platforms can use different API providers during parallel simulation, improving concurrency.

    Args:
        config: Simulation configuration dictionary
        use_boost: Whether to use accelerated LLM configuration (if available)
    """
    # Check if accelerated configuration exists
...
async def run_twitter_simulation(
    ...
    # Cap each agent's in-memory message history to prevent OOM crashes in long runs
    apply_memory_limits(result.agent_graph, "Twitter")

    # Get actual agent name mapping from config (use entity_name instead of default Agent_X)
    ...
    total_actions = 0
    last_rowid = 0  # Track last processed row ID in database (use rowid to avoid created_at format differences)
    ...
    log_info(f"Simulation loop completed! Elapsed time: {elapsed:.1f}s, total actions: {total_actions}")
...
async def run_reddit_simulation(
    ...
    # Reddit uses accelerated LLM configuration (if available, otherwise fallback to general configuration)
    ...
    # Cap each agent's in-memory message history to prevent OOM crashes in long runs
    apply_memory_limits(result.agent_graph, "Reddit")
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation completion, do not enter command waiting mode'
    )

    args = parser.parse_args()

    # Create shutdown event at the start of main function, ensure the whole program can respond to exit signals
    ...
    # Stage 4 Optimization: Log activity configuration
    log_manager.info(f"Stage 4 optimization configuration:")
    time_cfg = time_config.get('agents_per_hour_min', 5)
    time_cfg_max = time_config.get('agents_per_hour_max', 20)
    log_manager.info(f"  - Active agent range (per hour): {time_cfg}-{time_cfg_max}")
    log_manager.info(f"  - Expected activity rate: {(time_cfg_max / len(config.get('agent_configs', [1]))) * 100:.1f}% (peak hours)")
    log_manager.info(f"  - Expected total actions: ~{config_total_rounds * (time_cfg + time_cfg_max) // 2} (conservative estimate)")
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop completed! Total elapsed time: {total_elapsed:.1f}s")

    # Whether to enter command waiting mode
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command waiting mode - environment keeps running")
...
def setup_signal_handlers(loop=None):
    """
    Set up signal handlers to ensure correct exit when receiving SIGTERM/SIGINT

    Persistent simulation scenario: after simulation completion, do not exit, wait for interview commands
    When receiving termination signal, need to:
    1. Notify asyncio loop to exit waiting
    2. Give the program a chance to properly clean up resources (close databases, environments, etc.)
    3. Then exit
    """
    def signal_handler(signum, frame):
        ...
        # Don't directly call sys.exit(), let asyncio loop exit normally and clean up resources
